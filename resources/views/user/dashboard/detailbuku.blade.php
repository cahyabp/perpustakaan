...
<div class="w-full overflow-x-hidden border-t flex flex-col">
    <main class="w-full h-screen flex-grow p-6 px-10 relative min-h-screen">
        ...
        <form action="{{ route('usersaddBookToCart', ['id' => $book->id]) }}" method="POST">
            @method('POST')
            @csrf
            <div
                class="absolute bottom-0 left-0 z-50 w-full h-16 bg-gray-50 border-t border-gray-200 dark:bg-gray-700 dark:border-gray-600 flex items-center justify-center gap-5">
                ...
                <div date-rangepicker class="flex items-center">
                    <div class="relative">
                        ...
                        <input required name="tanggal_pinjam" type="text"
                    ...
                    <div class="relative">
                        ...
                        <input required name="tanggal_pengembalian" type="text"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 pl-10"
                            placeholder="Tanggal Pengembalian" @if($bukuBelumDikembalikan->count() > 0) disabled @endif
                        @if($cartFull) disabled @endif>
                    </div>
                </div>

                <button type="submit" @if($bukuBelumDikembalikan->count() > 0) disabled @endif
                    @if($cartFull) disabled @endif
                    class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none
                    focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center
                    dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Tambah
                    ke keranjang</button>
            </div>
        </form>
    </main>
